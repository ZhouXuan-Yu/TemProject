...
"""Capture bounded observations for successfully completed state-changing tools."""
...
def _summarize(tool_name: str, tool_input: Any) -> dict[str, Any]:
    config = load_config()
    runtime = config.get("runtime") if isinstance(config.get("runtime"), dict) else {}
    max_chars = int(runtime.get("max_observation_chars", 1600))

    result: dict[str, Any] = {"tool_name": tool_name, "status": "ok"}
...
def main() -> None:
    ...
    observation = _summarize(tool_name, payload.get("tool_input") or {})
